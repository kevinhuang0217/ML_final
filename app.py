...
st.title("🎨 商業簡報視覺配色 AI 推薦引擎")
st.markdown("##### 基於機器學習與受眾心理學，為您的商業提案與專案報告計算最具說服力的視覺搭配。")
st.divider()
...
# ==========================================
COUNTRY_MAP = {
    "tw": "Taiwan", "cn": "China", "hk": "Hong Kong", "mo": "Macau", "jp": "Japan", "kr": "South Korea", "sg": "Singapore", "my": "Malaysia", "id": "Indonesia", "th": "Thailand", "vn": "Vietnam", "ph": "Philippines", "in": "India", "au": "Australia", "nz": "New Zealand",
    "us": "United States", "ca": "Canada", "mx": "Mexico", "br": "Brazil", "ar": "Argentina", "cl": "Chile", "co": "Colombia", "pe": "Peru",
    "gb": "United Kingdom", "uk": "United Kingdom", "ie": "Ireland", "fr": "France", "de": "Germany", "es": "Spain", "it": "Italy", "pt": "Portugal", "nl": "Netherlands", "be": "Belgium", "ch": "Switzerland", "at": "Austria", "se": "Sweden", "no": "Norway", "fi": "Finland", "dk": "Denmark", "pl": "Poland", "ru": "Russia", "ua": "Ukraine", "gr": "Greece", "tr": "Turkey", "cz": "Czech Republic", "ro": "Romania",
    "ae": "United Arab Emirates", "sa": "Saudi Arabia", "il": "Israel", "eg": "Egypt", "za": "South Africa", "ng": "Nigeria"
}

LANG_MAP = {
    "en": "English", "zh": "Chinese", "es": "Spanish", "fr": "French", "de": "German", "ja": "Japanese", "ko": "Korean", "it": "Italian", "pt": "Portuguese", "ru": "Russian", "ar": "Arabic", "hi": "Hindi",
    "nl": "Dutch", "pl": "Polish", "sv": "Swedish", "da": "Danish", "fi": "Finnish", "no": "Norwegian", "el": "Greek", "he": "Hebrew", "ro": "Romanian", "hu": "Hungarian", "cs": "Czech", "uk": "Ukrainian",
    "ms": "Malay", "id": "Indonesian", "th": "Thai", "vi": "Vietnamese", "tl": "Tagalog", "bn": "Bengali", "ta": "Tamil", "ur": "Urdu"
}

def format_country(code):
    name = COUNTRY_MAP.get(code.lower(), "Unknown")
    return f"{code} ({name})" if name != "Unknown" else f"{code} ({code.upper()})"

def format_lang(code):
    name = LANG_MAP.get(code.lower(), "Unknown")
    return f"{code} ({name})" if name != "Unknown" else f"{code} ({code.upper()})"
...
with st.sidebar:
    ...
    fluent_val = st.slider("受眾英語流利度 (0-10)", 0, 10, 8)
...
st.markdown("請根據商業意圖調控以下維度。這將引導演算法為您錨定最佳副色。")
...
if st.button("✨ 啟動 AI 演算法計算最佳配色", use_container_width=True, type="primary"):
    with st.spinner("AI 引擎運算中 (貝氏機率分佈與退火尋優)..."):
        ...
        st.markdown(f"📊 **模型統計顯著性：** 隨機基準機率為 8.3%。本次 Top-3 推薦總共囊括了 **{top3_raw_sum:.1%}** 的後驗機率。")
